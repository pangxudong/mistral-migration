...
class ClusterAction(base.Action):
    ...
    def do_recover(self, cold_migrate = False):
        """Handler for the CLUSTER_RECOVER action.

        :returns: A tuple containing the result and the corresponding reason.
        """
#        if(cold_migrate==True):
#              try:
#                    reason = os.popen("mistral execution-create cluster-coldmigration cold_migration_input.json").read()
#              except Exception,e:
#                    LOG.error(str(e))
#        else:
        try:
              reason = os.popen("mistral execution-create cluster-livemigration live_migration_input.json").read()
        except Exception,e:
              LOG.error(str(e))
        return self.RES_OK, reason
